...
import { getScrapPlains } from "../../../utils/API/wasteland";
...
const ScrapPlains = () => {
  ...
  useEffect(() => {
    fetchScrapPlainsCollectibles();
  }, []);

  const fetchScrapPlainsCollectibles = async () => {
    try {
      const data = await getScrapPlains();
      setContent(data);
    } catch (err) {
      console.error(err);
  ...
  return (
    <div>
      <hr id="scrap-plains"></hr>
      <h3>▽ Scrap Plains Collectibles</h3>
