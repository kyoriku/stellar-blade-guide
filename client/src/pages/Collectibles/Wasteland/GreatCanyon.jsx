...
import { getGreatCanyon } from "../../../utils/API/wasteland";
...
const GreatCanyon = () => {
  ...
  useEffect(() => {
    fetchGreatCanyonCollectibles();
  }, []);

  const fetchGreatCanyonCollectibles = async () => {
    try {
      const data = await getGreatCanyon();
      setContent(data);
    } catch (err) {
      console.error(err);
  ...
  return (
    <div>
      <hr id="great-canyon"></hr>
      <h3>▽ Great Canyon Collectibles</h3>
