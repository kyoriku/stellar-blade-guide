...
import { getBarrenLands } from "../../../utils/API/wasteland";
...
const BarrenLands = () => {
  ...
  useEffect(() => {
    fetchBarrenLandsCollectibles();
  }, []);

  const fetchBarrenLandsCollectibles = async () => {
    try {
      const data = await getBarrenLands();
      setContent(data);
    } catch (err) {
      console.error(err);
  ...
  return (
    <div>
      ...
      <h3>▽ Barren Lands Collectibles</h3>
