...
import { getRottenLabyrinth } from "../../../utils/API/matrix11";
...
const RottenLabyrinth = () => {
  ...
  useEffect(() => {
    fetchRottenLabyrinthCollectibles();
  }, []);

  const fetchRottenLabyrinthCollectibles = async () => {
    try {
      const data = await getRottenLabyrinth();
      setContent(data);
    } catch (err) {
      console.error(err);
  ...
  return (
    <div>
      <Header id="rotten-labyrinth" title="▽ Rotten Labyrinth Collectibles" />
