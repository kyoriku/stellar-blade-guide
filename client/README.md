...
```
src/
  main.tsx           entry: ErrorBoundary > QueryClientProvider > Toast > Auth providers
  ...
  pages/             HomePage
                     six index pages (Walkthroughs/Levels/Collectibles/Upgrades/
                       Materials/Cosmetics), all thin wrappers over CategoryIndexPage
                     WalkthroughsListPage        walkthroughs of one mission type
                     WalkthroughDetailPage, LevelDetailPage, CollectibleTypeDetailPage
                                                 the three scroll-spy detail pages
                     Login / Register / OAuthCallback / ForgotPassword / ResetPassword /
                       Settings                  auth + account
                     Disclaimer / TermsOfService / PrivacyPolicy / BloodRainPage / ErrorPage
  components/        CategoryIndexPage (shared index page), CategoryCard, ImageGallery,
                     TableOfContents + FloatingTOC, WalkthroughContent, CollectibleSection,
                     SEO, StructuredData, PrefetchableLink, ProtectedRoute, Navbar, Footer,
                     comments/, navbar/, skeletons, modals
  hooks/             useCollectibles, useWalkthroughs, useProgress, useComments,
                     useNotifications, useSearch, usePrefetch, useActiveSection, useAuth
  ...
  constants/         navigation.ts, categoryImages.ts, typeDescriptions.ts
  utils/             cloudinary.ts, imageCache.ts, slugify.ts, toc.ts,
                     parseDescription.tsx, validateUsername.ts
public/
  robots.txt         crawler policy (blocks AI scrapers), sitemap reference
  sitemap.xml        static sitemap (regenerated by a server script)
  assets/favicon/    favicons wired in index.html
  assets/screenshots/  README screenshots
  assets/images/     local source screenshots (gitignored); production serves from Cloudinary
```
