...
async def types_db_session(types_db_engine):
    session_factory = async_sessionmaker(types_db_engine, expire_on_commit=False)
...
async def test_get_all_types_with_data(types_client, types_db_session):
    ...
    types_db_session.add_all([
        CollectibleType(id=1, name="Sword"),
        CollectibleType(id=2, name="Nano Suit"),
    ])
    ...
    data = response.json()
    assert len(data) == 2
    assert data[0]["name"] == "Nano Suit"
    assert data[1]["name"] == "Sword"
...
async def test_response_shape(types_client, types_db_session):
    ...
    types_db_session.add(CollectibleType(id=1, name="Nano Suit"))
    await types_db_session.commit()

    response = await types_client.get("/api/types/")
    item = response.json()[0]

    assert set(item.keys()) == {"id", "name", "category_group", "display_order"}
...
async def test_cache_hit_skips_db(types_client, fake_redis):
    ...
    cached_data = [{"id": 99, "name": "Cached Type", "category_group": None, "display_order": None}]
